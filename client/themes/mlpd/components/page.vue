        ...
        v-toolbar(:color='$vuetify.theme.dark ? `grey darken-4-d3` : `grey lighten-3`', flat, dense, v-if='$vuetify.breakpoint.smAndUp')
          v-breadcrumbs.breadcrumbs-nav.pl-0(
            ...
            template(slot='item', slot-scope='props')
              v-icon(v-if='props.item.path.path === "/"', small, @click='goHome') mdi-home
              v-btn.ma-0(v-else, :href='props.item.path', small, text) {{props.item.item.name}}
          ...
          // 夜间模式切换按钮
          v-tooltip(bottom, :content='$vuetify.theme.dark ? $t("common:theme.lightMode") : $t("common:theme.darkMode")')
            v-btn(
              icon
              small
              @click='$vuetify.theme.dark = !$vuetify.theme.dark'
              :aria-label='$vuetify.theme.dark ? $t("common:theme.switchToLight") : $t("common:theme.switchToDark")'
              class='ml-2'
              )
              v-icon(:color='$vuetify.theme.dark ? `yellow--text text--lighten-3` : `grey--text text--darken-3`')
                | {{ $vuetify.theme.dark ? 'mdi-sun' : 'mdi-moon' }}
        v-divider
                ...
                v-tooltip(bottom, v-if='$vuetify.breakpoint.smAndDown')
                  template(v-slot:activator='{ on }')
                    v-btn(
                      ...
                      v-icon(:color='$vuetify.theme.dark ? `yellow--text` : `grey--text`')
                        | {{ $vuetify.theme.dark ? 'mdi-sun' : 'mdi-moon' }}
...
// 夜间模式切换按钮样式优化
.theme--light .v-btn[aria-label*="dark mode"] .v-icon {
  color: #424242; // 浅色主题下图标深色
}
.theme--dark .v-btn[aria-label*="light mode"] .v-icon {
  color: #FFD700; // 深色主题下图标金色（更醒目）
}
